 ...
 * @return {string}
 */
Finalizer.prototype.download = function(project, firstTime, dependencies, callback) {
    ...
    Project.findOne({ where: { slug: projectSlug }}, function(err, project) {
        ...
        Build.findOne({ where: { project_id: project.id }, order: 'created_at DESC'}, function(err, build) {
            ...
            if (fileHasChanged || firstTime) {
                return callback(null, './storage/' + project.slug + '/' + build.hash + '/compressed.tar.gz');
            } else {
