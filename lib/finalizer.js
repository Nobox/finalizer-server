 ...
 * @return {string}
 */
Finalizer.prototype.download = function(project, dependencies, callback) {
    ...
    Project.findOne({ where: { slug: projectSlug }}, function(err, project) {
        ...
        Build.findOne({ where: { project_id: project.id }, order: 'created_at DESC'}, function(err, build) {
            const archived = JSON.parse(fs.readFileSync('./storage/' + project.slug + '/' + build.hash + '/package.json'));
            const newDependencies = dependencies;
            const fileHasChanged = diff(archived, newDependencies);
            if (fileHasChanged) {
                callback(null, './storage/' + project.slug + '/' + build.hash + '/compressed.tar.gz');
            } else {
                callback('Nothing to download, you have the most updated build =)');
                return;
            }
        });
    });
